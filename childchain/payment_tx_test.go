...
//test build, sign and submit payment transaction via transaction.create works properly
func TestBuildSignPaymentTx(t *testing.T) {
	...
	blknumWanted := json.Number(strconv.Itoa(123000))
	...
	// test signer
	signerCalled := false
	mocksigner := func (in []byte)([][]byte, error) {
		signerCalled = true
		return [][]byte{}, nil
	}
	_, err = paymenttx.SignTransaction(mocksigner)
	if !signerCalled {
		t.Error("Signer function did not get called")
	}
	if err != nil {
		t.Errorf("Expected no error, got %v", err)
	}
